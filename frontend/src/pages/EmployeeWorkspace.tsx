...
import { isCompletionBusy } from '../types/completion'

export function EmployeeWorkspace() {
  const { employeeId = '' } = useParams()
  const { employee: profile, career, snapshotMismatch, snapshotRefresh, refreshSnapshot } = useEmployeeDashboard(employeeId)
  const completion = useActivityCompletion(employeeId)
  const labels = copy.dashboard
  const profileAccess = getAccessIssue(profile.error, 'employee')
  const careerAccess = getAccessIssue(career.error ?? snapshotRefresh.error, 'career')

  if (profile.isPending) return <><BackLink /><LoadingPanel /></>
  if (!profile.data || profileAccess) {
    const missing = profile.error instanceof ApiError && profile.error.code === 'NOT_FOUND'
    return <>
      <BackLink />
      <StatePanel error title={profileAccess?.title ?? (missing ? copy.employeeMissing : copy.errorTitle)}
        description={profileAccess?.description ?? (missing ? copy.employeeMissingDescription : copy.errorDescription)}>
        {!missing && !profileAccess && <Button disabled={profile.isFetching} onClick={() => void profile.refetch()}>{profile.isFetching ? copy.retrying : copy.retry}</Button>}
      </StatePanel>
    </>
  }
  ...
  return (
    <div className={`employee-dashboard ${completion.state.phase === 'success' ? 'dashboard-refreshed' : ''}`}>
      ...
      <EmployeeProfile employee={employee} target={snapshotMismatch || careerAccess ? undefined : overview?.target} isTargetPending={career.isPending || snapshotRefresh.isFetching} />
      {career.isPending ? <LoadingPanel label={labels.careerLoading} /> : snapshotMismatch && !careerAccess ?
        <StatePanel error title={labels.snapshotChanged} description={snapshotRefresh.isError ? labels.snapshotRefreshFailed : labels.snapshotChangedDescription}>
          <Button disabled={snapshotRefresh.isFetching || profile.isFetching || career.isFetching || isCompletionBusy(completion.state) || Boolean(completion.blockedReason)}
            aria-busy={snapshotRefresh.isFetching || undefined} onClick={() => void refreshSnapshot()}>
            {snapshotRefresh.isFetching ? labels.snapshotRefreshing : labels.snapshotRefresh}
          </Button>
        </StatePanel> : !overview || careerAccess ?
        <StatePanel error title={careerAccess?.title ?? labels.careerError} description={careerAccess?.description ?? labels.careerErrorDescription}>
          {!careerAccess && <Button disabled={career.isFetching} onClick={() => void career.refetch()}>{career.isFetching ? copy.retrying : copy.retry}</Button>}
        </StatePanel> : <>
          ...
          <AssessmentNotes overview={overview} />
