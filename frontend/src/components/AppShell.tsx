...
import { sessionHome, setSession } from '../services/session'
...
export function AppShell() {
  ...
  const home = session ? sessionHome(session) : '/'
  const modeLabel = apiMode === 'mock' ? 'Mock preview' : 'Live API'
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${menuOpen ? 'sidebar-open' : ''}`}>
      ...
      <nav aria-label={copy.workspace} id="workspace-navigation">
        <Link to={home} className={`nav-item ${pathname === '/' || pathname.startsWith('/employees/') ? 'active' : ''}`}
          aria-current={pathname === '/' || pathname.startsWith('/employees/') ? 'page' : undefined} onClick={() => setMenuOpen(false)}>
          <Icon name="grid" />{session?.role === 'hr' ? 'Employee directory' : copy.employeeNav}
        </Link>
        {session?.role === 'hr' && <NavLink to="/hr" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} onClick={() => setMenuOpen(false)}>
          <Icon name="chart" />{copy.hrNav}
        </NavLink>}
      </nav>
      <div className="sidebar-message"><Icon name="spark" /><p>{copy.tagline}</p><span>{copy.footerDetail}</span></div>
      <div className="sidebar-bottom"><span className="status-dot" /><span>{modeLabel}</span></div>
    </aside>
    <div className="workspace-content">
      <header className="topbar">
        ...
        <div className="topbar-actions"><span className="demo-badge"><span className="status-dot" />{modeLabel}</span>{session?.role === 'hr' && <EmployeeSelector />}<div className="identity-actions"><span className="identity-label">{session?.role === 'hr' ? 'HR workspace' : session?.employeeId}</span><button className="identity-switch" type="button" onClick={() => setSession(null)}>Change identity</button></div></div>
      </header>
