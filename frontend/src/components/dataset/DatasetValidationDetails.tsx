  ...
  return <section className="dataset-issue-section" aria-labelledby={id}>
    ...
    <ol className="dataset-issues" aria-label={title}>
      {issues.slice(0, visible).map((issue, index) => <li key={index}>
        <dl>{([[datasetCopy.code, issue.code], [datasetCopy.location, issue.location], [copy.dataset.file, issue.file], [copy.dataset.row, issue.row], [copy.dataset.record, issue.record], [copy.dataset.field, issue.field]] as const)
...
export function DatasetValidationDetails({ result }: { result: DatasetValidationResult }) {
  const entries = (['employees', 'skills', 'roleProfiles', 'events', 'activityHistory', 'recordsProcessed'] as const)
  ...
  return <div className="dataset-validation-details">
    {result.valid === true && <section aria-label={copy.dataset.summary}>
      ...
      {entries.length ? <dl className="dataset-summary">{entries.map((key) => {
        ...
        return <div key={key}><dt>{key === 'roleProfiles' ? datasetCopy.roleProfiles : copy.dataset[key]}</dt><dd>{isAggregateValue(value) ? formatAggregate(value) : copy.dataset.noSummary}</dd></div>
